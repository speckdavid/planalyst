...
#include "zPRINTDATE"
  ...
  if(argc == 1) {
    ...
    printf("-1     Force at least one action at every step when not in the goal state.\n");
    printf("-2     Force at least one action at every step.\n");
    printf("-3     Force no actions in steps after a step without an action.\n");
  ...
  for(i=1;i<argc;i++) {
	...
	printf(" -E");
	break;
      case '1': /* At least one action every step where not in goal state. */
	flagAtLeastOneAction = 1;
	printf(" -2");
	break;
      case '2': /* Always at least one action every step. */
	flagAtLeastOneAction = 2;
	printf(" -2");
  break;
      case '3': /* Not allowed to apply an action after a time step is skipped. */
	flagAtLeastOneAction = 3;
	printf(" -3");
	break;
      default:
	printf("ignoring %s\n",argv[i]);
