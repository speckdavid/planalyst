...
/* How many bits are needed for expressing numbers up to n? */
...
void SEQUmutexes() {
  ...
  indexbits = bitsneeded(nOfActions - 1);
...
#define RANDOMORDER
    ...
    if(flagAtLeastOneAction == 1) {
      /* Force at least one action at every step when not in the goal state. */
      int aux = allocAUX(1);
      fmalist *allActions;
      int i;
      allActions = NULL;
    ...
    } else if (flagAtLeastOneAction == 2) {
      /* Force at least one action at every step. */
      fmalist *allActions;
      allActions = NULL;
      int i;
      for(i=0;i<nOfActions;i++) allActions = fmacons(fmaOP(i),allActions);
      addformula(transition,Fdisj(allActions));
    } else if (flagAtLeastOneAction == 3) {
      /* Force at least one action at every step. */
      int aux = allocAUX(1); // true if no action has been selected
      fmalist *allActions;
      allActions = NULL;
      int i;
      for(i=0;i<nOfActions;i++) {
        addformula(transition,Fdisj2(Fneg(fmaOP(i)),Fnatom(AUX(aux))));
        allActions = fmacons(fmaOP(i),allActions);
      }
      addformula(transition,Fdisj(fmacons(Fatom(AUX(aux)),allActions)));
      addformula(transition,Fimpl(Fatom(AUX(aux)),Fatom(AUX(NEXT(aux)))));
      
      
      // HACK: We fix the aux variables of the sequential encoding to false
      // if no action has been selected
      if (planSemantics == Sequ) {
        int indexbits = bitsneeded(nOfActions - 1);
        int seq_mutex_aux_start = aux - indexbits;
        for (int i = seq_mutex_aux_start; i < aux; ++i) {
          addformula(transition, Fimpl(Fatom(AUX(aux)), Fnatom(AUX(i))));
        }
      }
    }
...
long getFileSize(char *filename) {
    ...
    return file_status.st_size / (1024 * 1024);
}
...
void outputDIMACS() {
  ...
  for(i=firstTimePoint;i<=lastTimePoint;i+=outputTimeStep) { /* Write files for different lengths. */
...
#endif
    int num_vars = (nOfAtoms-1)+(i*varsPerTime)+1;
    int num_clauses = nOfClauses+(i-1)*nOfTClauses+i*nOfInvariants/2;
    if (flagAtLeastOneAction == 3) --num_clauses; // We need to skip the last clause which refers to NEXT(AUX)
    ...
    for(j=0;j<=i;j++) { /* Go through time points for one formula. */
      float fileSize = getFileSize(filename);
      if (fileSize >= flagMemoryLimit) {
        fclose(F);
        fprintf(stderr, "\nFile size exceeds %.0fMB. Abort.\n", fileSize, flagMemoryLimit);
        exit(1);
      }
  ...
  if (with) {
    ...
    // Check all entries in one pass and prepare output if valid
    for (h = ptr; h < ptr + len; h++) {
        int cur_bias = (clauseset[h] < 0) ? -bias : bias;

        // Check if the clause plus bias exceeds num_vars
        if (clauseset[h] + cur_bias > num_vars) {
            flagSkip = 1;
            break; // Stop further processing if any invalid clause is found
        }
    }

    // If no invalid clauses were found, proceed to output
    if (!flagSkip) {
        for (h = ptr; h < ptr + len; h++) {
            if (clauseset[h] < 0) {
